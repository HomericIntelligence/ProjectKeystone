...
TEST(E2E_PhaseA, AsyncDelegationWithWorkStealing) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        std::cout << "  → Sent command " << (i+1) << " to " << target->getAgentId()
                  << ": " << num1 << " + " << num2 << " = " << expected << std::endl;
    }
    ...
    for (const auto& resp : responses) {
        ...
        try {
            ...
            if (result == expected) {
                std::cout << "  ✓ Command for msg_id " << resp.msg_id.substr(0, 8) << "... succeeded: got " << result << std::endl;
                successful++;
            } else {
                std::cout << "  ✗ Command for msg_id " << resp.msg_id.substr(0, 8) << "... incorrect: got " << result
...
TEST(E2E_PhaseA, WorkStealingLoadBalancing) {
    ...
    std::cout << "Load distribution: " << agents_with_messages << "/10 agents received work" << std::endl;
