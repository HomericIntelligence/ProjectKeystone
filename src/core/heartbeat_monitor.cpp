...
HeartbeatMonitor::HeartbeatMonitor(Config config)
    : config_(config)
{
    Logger::info("HeartbeatMonitor: Created (interval={}ms, timeout={}ms)",
                 config_.heartbeat_interval.count(), config_.timeout_threshold.count());
}
    ...
    if (it == agents_.end()) {
        ...
        if (was_dead) {
            ...
            Logger::trace("HeartbeatMonitor: Heartbeat from {} (total={})",
                         agent_id, it->second.total_heartbeats);
        }
    }
}
...
std::optional<HeartbeatMonitor::AgentStatus> HeartbeatMonitor::getStatus(const std::string& agent_id) const {
    std::lock_guard<std::mutex> lock(agents_mutex_);
