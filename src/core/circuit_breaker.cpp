    ...
    : config_(config)
{
    Logger::info("CircuitBreaker: Created (failure_threshold={}, timeout={}ms, success_threshold={})",
                 config_.failure_threshold, config_.timeout_ms.count(), config_.success_threshold);
}
...
void CircuitBreaker::recordSuccess(const std::string& target_id) {
    ...
    Logger::trace("CircuitBreaker: Success for {} (consecutive={})",
                 target_id, status.consecutive_successes);
...
void CircuitBreaker::recordFailure(const std::string& target_id) {
    ...
    Logger::debug("CircuitBreaker: Failure for {} (consecutive={})",
                 target_id, status.consecutive_failures);
...
std::optional<CircuitBreaker::CircuitStatus> CircuitBreaker::getStatus(const std::string& target_id) const {
    std::lock_guard<std::mutex> lock(circuits_mutex_);
