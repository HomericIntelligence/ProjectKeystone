...
// Benchmarks for resilience components: retry policy, circuit breaker, heartbeat
//
// Measures:
...
static void BM_RetryPolicy_FullSequence(benchmark::State& state) {
    ...
    for (auto _ : state) {
        auto policy = RetryPolicy(max_retries, std::chrono::milliseconds(10), 2.0, std::chrono::seconds(10));
...
static void BM_RetryPolicy_VaryingMultiplier(benchmark::State& state) {
    ...
    auto policy = RetryPolicy(20, std::chrono::milliseconds(100), multiplier, std::chrono::seconds(30));
...
static void BM_CircuitBreaker_StateTransition(benchmark::State& state) {
    ...
    for (auto _ : state) {
        auto cb = CircuitBreaker("test", failure_threshold, std::chrono::seconds(10), std::chrono::seconds(5));
